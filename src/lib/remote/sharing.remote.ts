...
export const shareFile = command(
  ...
  async ({ itemId, isPublic, emails, expiresAt }) => {
    ...
    try {
      ...
      await invalidateCache(`getFileShare:${file.storageKey}`);
...
export const shareFolder = command(
  ...
  async ({ itemId, isPublic, emails, expiresAt }) => {
    ...
    try {
      ...
      await invalidateCache(`getFolderShare:${folder.id}`);
...
export const getFileShare = query(
  ...
  async ({ itemId }) => {
    return withCache(`getFileShare:${itemId}`, async () => {
      const {
        locals: { user },
      } = getRequestEvent();
      if (!user) {
      ...
      const [file] = await db
        ...
        .where(
          and(
            eq(table.file.storageKey, itemId),
...
export const getFolderShare = query(
  ...
  async ({ itemId }) => {
    return withCache(`getFolderShare:${itemId}`, async () => {
      const {
        locals: { user },
      } = getRequestEvent();
      if (!user) {
